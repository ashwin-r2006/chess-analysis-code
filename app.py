...
def generate_fen(data):
    ...
    def place_pieces(coords, piece):
        for sq in coords:
            ...
            if sq not in position:
                if len(sq)%2==0:
                    co=[sq[i-1:i+1] for i in range(len(sq)) if i%2!=0]
                    if not all(len(s) == 2 and "a" <= s[0] <= "h" and "1" <= s[1] <= "8" for s in co):
                        return f"incorrect coordinate: {sq}"
                    er=place_pieces(co, piece)
                    if er:
                        return er
                    continue
                return f"incorrect coordinate: {sq}"
    ...
    if len(kw) != 1 or len(kb) != 1:
        return " only 1 king of each colour can be present"
